...
(function(){
    ...
    // save Backbone value, incase 2 backbone are in page.
    var previousBackbone = root.Backbone;

    // local reference to splice
    var splice = Array.prototype.splice;

    // Top level namespace
    // All public Backbone classes & modules attach to this.
    // Exported for both CommonJS & browser
    var Backbone;
    ...
    // Current version of library. Keep in sync with 'package.json'
    ...
    if( !_
        ...
        // we're on server, require underscore
        _ = require('underscore');
    }

    // jQuery, Zepto, Ender own '$' variable
    Backbone.$ = root.jQuery || root.Zepto || root.ender;

    // Run Backbone.js in *noConflict* mode,
    // return 'Backbone' variable to prev owner
    // Returns a ref to this Backbone object
    Backbone.noConflict = function() {
        // restore 'window' or 'global' Backbone
    ...
    // HTTP 'PUT' and 'DELETE' requests will be faked using
    // '_method' param and 'X-Http-Method-Override' header will be set
    Backbone.emulateHTTP = false;

    // Turn on 'emulateJSON' to support legacy servers that can't deal
    // with direct 'application/json' requests.
    // Body will be encoded as 'application/x-www-form-urlencoded' and
    // model will be sent in form param named 'model'
    Backbone.emulateJSON = false;

    // Regex used to split event strings
    var eventSplitter = /\s+/;


    var Events = Backbone.Events = {

        // Bind one or more space separated events, 'events', to a 'callback' function.
        // Passing 'all' will bind callback to all events fired
        on: function(events, callback, context) {
                ...
                while( event = events.shift() ) {
                    // get saved event callbacks
                    list = calls[event]
                           || ( calls[event] = [] ); // or init new callback array

                    // and push callback & context
        ...
        // If 'callback is null, removes all callbacks for event
        ...
        off: function( events, callback, context ) {
                ...
                // remove all callbacks by calling 'off()' with no param
                ...
                events = events
                            ? events.split( eventSplitter ) // get events from string
                            : _.keys(calls);                // or get them from _callbacks keys

                // Loop through callback list, splicing where appropriate
                while( event = events.shift() ) {
                    if( !(list = calls[event])  // callback(s) set for event
                        || !(callback || context) ) // no callback nor context defined
                    {

                        delete calls[event]; // delete all callbacks for event
                        continue;
                    }

                    // it's always a pair of callback & context
                    // that's why -2 or -= is used here
...
//                                    && list[i+1] !== context )) {
        ...
        // Callbacks are passed same arguments as 'trigger' is (without event name), other than 'all',
        // which causes callback to receive true name of event as 1st arg.
        trigger: function(events) {
            ...
            // Fill up 'rest' with callback arguments.
            // Copy tails of arguments, so loop is much faster than Array#slice
            for( i=1, length = arguments.length; i<length; i++ ) {
                rest[i-1] = arguments[i];
            }

            // For each event, walk through list of callbacks twice,
            // 1st to trigger event,
            // 2nd to trigger 'all' callbacks
            while( event = events.shift() ) {
